...
CID = string.sub(text, string.find(text,"%u%u%u"))
MOUNT = string.sub(text, string.find(text,"%d+")) or "1"

if inTable(CIDS, CID) then
  response = http.request {
    url="http://www.freecurrencyconverterapi.com/api/v3/convert?q="..CID.."_CNY&compact=ultra"
  }
  result = json.parse(response.content)[CID.."_CNY"] * tonumber(MOUNT)
  return {text=MOUNT.." "..CID.." = "..result.." CNY"}
else
  return {text="输入格式不对，正确示例： 10 USD"}
end
